<?php

use think\migration\Migrator;
use think\migration\db\Column;

class CreateGoodsRecommend extends Migrator
{
    /**
     * Change Method.
     *
     * Write your reversible migrations using this method.
     *
     * More information on writing migrations is available here:
     * http://docs.phinx.org/en/latest/migrations.html#the-abstractmigration-class
     *
     * The following commands can be used in this method and Phinx will
     * automatically reverse them when rolling back:
     *
     *    createTable
     *    renameTable
     *    addColumn
     *    renameColumn
     *    addIndex
     *    addForeignKey
     *
     * Remember to call "create()" or "update()" and NOT "save()" when working
     * with the Table class.
     */
    public function change()
    {
        $this->table('goods_recommend', ['comment' => '主推产品表'])
            ->addColumn('goods_id', 'integer', ['comment' => '商品ID'])
            ->addColumn('item_id', 'integer', ['default' => 0, 'comment' => '商品规格ID'])
            ->addColumn('image', 'string', ['default' => '', 'comment' => '主推图片'])
            ->addColumn('video', 'string', ['default' => '', 'comment' => '主推视频'])
            ->addColumn('video_cover', 'string', ['default' => '', 'comment' => '视频封面图'])
            ->addColumn('video_axis', 'integer', ['default' => 1, 'comment' => '视频轴方向：1横向型 2竖向型'])
            ->addColumn('sort', 'integer', ['default' => 0, 'comment' => '排序'])
            ->addColumn('is_open', 'integer', ['default' => 1, 'limit' => \Phinx\Db\Adapter\MysqlAdapter::INT_TINY,
                'comment' => '是否开启'])
            ->addColumn('add_time', 'integer')
            ->create();
    }
}
