...
class weixinApplet
{
    ...
    public function __construct()
    {
        ...
        $paymentPlugin = M('Plugin')->where(['code' => 'weixinApplet', 'type' => 'payment'])->find(); // 找到微信支付插件的配置
     ...
     */
    public function get_code($order, $openId)
    {
...
//        $openId = $_SESSION['openid'];
        ...
        $input->SetAttach('weixinApplet');
        $input->SetOut_trade_no($order['order_sn']);
        $input->SetTotal_fee($order['order_amount'] * 100);
        $input->SetTime_start(date('YmdHis'));
        $input->SetTime_expire(date('YmdHis', time() + 600));
        $input->SetGoods_tag('tp_wx_applet_pay');
        $input->SetTrade_type('JSAPI');
        $input->SetNotify_url(SITE_URL . '/index.php/Home/api.Pay/notifyUrl/pay_code/weixinApplet');
        ...
        $tools = new JsApiPay();
        $AppParameters = $tools->GetJsApiParameters($order2); // 二次签名
