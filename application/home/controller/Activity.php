...
class Activity
{
    ...
    public function groupBuyList()
    {
        ...
        $list = $GroupBuy
            ->alias('gb')
            ->field('gb.*
            ,FROM_UNIXTIME(start_time,"%Y-%m-%d") as start_time
            ,FROM_UNIXTIME(end_time,"%Y-%m-%d") as end_time
            ,(FORMAT(buy_num%group_goods_num/group_goods_num,2)) as percent
            ,CASE can_integral > 0  WHEN 1 THEN gb.price - g.exchange_integral ELSE gb.price END AS price
            ,CASE buy_num >= goods_num  WHEN 1 THEN 1 ELSE 0 END AS is_sale_out
            ,group_goods_num - buy_num%group_goods_num as people_num
    ...
    public function flashSaleList()
    {
        ...
        $FlashSale = new FlashSale();
        $flash_sale_goods = $FlashSale->alias('fl')->join('__GOODS__ g', 'g.goods_id = fl.goods_id')->with(['specGoodsPrice'])
            ->field('fl.*
            ,FROM_UNIXTIME(start_time,"%Y-%m-%d %H:%i:%s") as start_time
            ,FROM_UNIXTIME(end_time,"%Y-%m-%d %H:%i:%s") as end_time
            ,end_time as end_time_timestamp
            ,100*(FORMAT(buy_num/goods_num,2)) as percent
            ,CASE can_integral > 0  WHEN 1 THEN fl.price - g.exchange_integral ELSE fl.price END AS price
            ,g.goods_name
            ,g.original_img
            ,g.shop_price
            ,g.give_integral
