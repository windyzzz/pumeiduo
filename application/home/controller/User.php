...
class User
{
    ...
    public function account()
    {
        ...
        $order_sn = I('order_sn');
        $logic = new UsersLogic();
        $data = $logic->get_account_log_pc($this->user_id, $type, $order_sn);
    ...
    public function userWealth()
    {
        ...
        $return['has_pwd'] = $user['password'] ? 1 : 0;
        $return['order_type1_num'] = M('order')->where([
            'order_status' => ['in', [0, 1]],
            'pay_status' => 0,
            'user_id' => $user['user_id'],
        ])->count(); //待付款订单
        $return['order_type2_num'] = M('order')->where([
            'order_status' => ['in', [0, 1]],
            'pay_status' => 1,
            'shipping_status' => 0,
            'user_id' => $user['user_id'],
        ])->count(); //待发货订单
        $return['order_type3_num'] = M('order')->where([
            'order_status' => ['in', [1, 2, 4]],
            'pay_status' => 1,
            'shipping_status' => 1,
            'user_id' => $user['user_id'],
        ])->count(); //已发货订单
        $return['order_type4_num'] = M('order')->where([
            'order_status' => 4,
            'pay_status' => 1,
            'shipping_status' => 1,
            'user_id' => $user['user_id'],
        ])->count(); //已完成订单
        $return['coupon_num'] = M('coupon_list')->where([
            'uid' => $this->user_id,
            'status' => 0,
        ])->count(); //优惠券数量
    ...
    public function couponAllList()
    {
        $type = I('type', 0);
        $logic = new UsersLogic();
        $data = $logic->get_coupons($this->user_id, I('type'));
        foreach ($data['result'] as $k => $v) {
            $user_type = $v['use_type'];
            $data['result'][$k]['use_scope'] = C('COUPON_USER_TYPE')["$user_type"];
            if (1 == $user_type) { //指定商品
    ...
    public function mobile_validate(Request $request)
    {
        if ($request->isPost()) {
            ...
            $logic = new UsersLogic();
            if (1 == $step) {
                if ($old_mobile != $user_info['mobile']) {
                ...
                return json(['status' => 1, 'msg' => '手机号码修改成功', 'result' => null]);
            }

            return json(['status' => 0, 'msg' => '请求接口非法', 'result' => null]);
        }

        return json(['status' => -1, 'msg' => '请求接口非法.', 'result' => null]);
    }
    ...
    {
        $userLogic = new UsersLogic();
        $data = $userLogic->get_goods_collect_pc($this->user_id);
        $return = [];
        $page = $data['page']; // 赋值分页输出
        $return['lists'] = $data['result'];
        $return['active'] = 'goods_collect';

        return json(['status' => 1, 'msg' => '获取成功', 'result' => $return, 'page' => $page]);
    }
    ...
    public function setPassword(Request $request)
    {
        ...
        if ($request->isPost()) {
            $userLogic = new UsersLogic();
            $data = $userLogic->setPassword($this->user_id, I('post.new_password'), I('post.confirm_password')); // 获取用户信息
            if (-1 == $data['status']) {
    ...
    public function changeType()
    {
        ...
        if (1 == $type) {
            $pay_points = tpCache('basic.reg_integral'); // 会员注册赠送积分
            if ($pay_points > 0) {
                accountLog($this->user['user_id'], 0, $pay_points, '会员注册赠送积分'); // 记录日志流水
            }
            ...
            if ($will_invite_uid > 0) {
                ...
                // 邀请送积分
                $invite_integral = tpCache('basic.invite_integral');
                accountLog($will_invite_uid, 0, $invite_integral, '邀请用户奖励积分');
        ...
        return json(['status' => 1, 'msg' => '新账号认证成功', 'result' => null]);
    }
...
//        $return['page'] =  $Page;
        ...
        return json(['status' => 1, 'msg' => 'success', 'result' => $return, 'page' => $Page]);
    }
    ...
    public function electronic()
    {
        ...
        $order_sn = I('order_sn');
        $logic = new UsersLogic();
        $data = $logic->get_electronic_log_pc($this->user_id, $type, $order_sn);
        $return = $data['result'];
    ...
    public function exchangeElectronic()
    {
        ...
        accountLog($user['user_id'], 0, 0, '用户余额转电子币', 0, 0, '', $electronic);
        accountLog($user['user_id'], -$electronic, 0, '电子币充值（余额转）', 0, 0, '', 0);
    ...
    public function recharge(Request $request)
    {
        ...
        $type = I('type');
        $Userlogic = new UsersLogic();
        $result = $Userlogic->get_money_log_pc($this->user_id, $type);  //用户资金变动记录
    ...
    public function bindOldUser(Request $request)
    {
        ...
        M('OauthUsers')->where('user_id', $this->user_id)->update(['user_id' => $user['user_id']]);
        // M('couponList')->where('uid',$this->user_id)->update(array('uid'=>$user['user_id']));

        M('Users')->where('user_id', $user['user_id'])->update($user_data);

        //冻结新账户
        M('Users')->where('user_id', $this->user_id)->update(['is_lock' => 1]);
    ...
    public function getWithdrawals()
    {
        ...
        if ($screen_times) {
            ...
            $end = strtotime($end);
            $account_log_where['create_time'] = ['between', [$start, $end]];
        }
        $account_log_where['user_id'] = $this->user_id;
        $count = M('Withdrawals')->where($account_log_where)->count();
        ...
        $return['WITHDRAW_STATUS'] = C('WITHDRAW_STATUS');
     ...
     * 2017/9/28.
     */
    public function signPc()
    {
        $userLogic = new UsersLogic();
        $user_id = $this->user_id;
        $info = $userLogic->idenUserSignPc($user_id); //标识签到
        $return = $info;
    ...
    public function getReward()
    {
        ...
        if ($reward) {
            if ($reward['reward_integral'] > 0 || $reward['reward_electronic'] > 0) {
                accountLog($this->user_id, 0, $reward['reward_integral'], '用户领取任务奖励', 0, 0, $reward['order_sn'], $reward['reward_electronic']);
            }
     ...
     * @return \think\response\Json
     */
    public function integralSummary()
    {
        $three_month_ago = strtotime('-3 month');
        $shopping_return = M('account_log')->where([
            'user_id' => $this->user_id,
            'type' => 5,
            'change_time' => ['egt', $three_month_ago],
        ])->sum('pay_points');
        $other_return = M('account_log')->where([
            'user_id' => $this->user_id,
            'type' => ['neq', 5],
            'change_time' => ['egt', $three_month_ago],
        ])->sum('pay_points');
        $return['shopping_return'] = $shopping_return;
        $return['other_return'] = $other_return;

        return json(['status' => 1, 'msg' => 'ok', 'result' => $return]);
    }
}
