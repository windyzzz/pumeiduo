...
//        $return['page'] = $show;
    ...
    public function return_goods(Request $request)
    {
        ...
        $order['city_name'] = M('region2')->where('id', $order['city'])->getField('name');
        $order['district_name'] = M('region2')->where('id', $order['district'])->getField('name');
        $order['province_name'] = M('region2')->where('id', $order['province'])->getField('name');
        $return['order'] = $order;
...
//        $return['page'] =  $page->show();// 赋值分页出
    ...
    public function return_goods_info(Request $request)
    {
        ...
        if ($return_goods['refund_time']) {
            $return_goods['refund_time'] = date('Y-m-d H:i:s', $return_goods['refund_time']);
        }
        if ($return_goods['checktime']) {
            $return_goods['checktime'] = date('Y-m-d H:i:s', $return_goods['checktime']);
        }
        if ($return_goods['receivetime']) {
            $return_goods['receivetime'] = date('Y-m-d H:i:s', $return_goods['receivetime']);
        }
        if ($return_goods['canceltime']) {
            $return_goods['canceltime'] = date('Y-m-d H:i:s', $return_goods['canceltime']);
        }
