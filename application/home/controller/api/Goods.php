...
//        }
        ...
        if (!empty($flashSale)) {
            ...
            $groupBuy = Db::name('group_buy gb')
                ->join('spec_goods_price sgp', 'sgp.item_id = gb.item_id', 'LEFT')
                ->where(['gb.goods_id' => $goods_id, 'gb.start_time' => ['<=', time()], 'gb.end_time' => ['>=', time()], 'gb.is_end' => 0])
...
//        $filter_attr = $goodsLogic->get_filter_attr($filter_goods_id, $filter_param, 'goodsList', 1); // 获取指定分类下的筛选属性
        ...
        if ($count > 0) {
            // 获取商品数据
            $goodsLogic = new GoodsLogic();
            $goodsData = $goodsLogic->getGoodsList($filter_goods_id, $sortArr, $page, $this->user);
        }
...
//        }
        ...
        // 获取商品数据
        $goodsLogic = new GoodsLogic();
        $goodsList = $goodsLogic->getGoodsList($goodsIds, $sortArr, $page, null)['goods_list'];
