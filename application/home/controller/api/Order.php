...
//        }
        ...
        foreach ($orderList as $k => $list) {
            ...
            $orderData[$k] = [
                ...
                'order_info' => [
                    ...
                    'order_amount' => $list['order_type'] == 5 ? $list['school_credit'] : $list['order_amount'],
            ...
            foreach ($orderGoods as $goods) {
                if ($list['order_id'] == $goods['order_id']) {
                    $orderData[$k]['order_goods'][] = [
                        ...
                        'exchange_price' => $list['order_type'] == 5 ? $goods['school_credit'] : $goods['member_goods_price'],
    ...
    public function order_detail_new()
    {
        ...
        $orderData = [
            ...
            'order_amount' => $orderInfo['order_type'] == 5 ? $orderInfo['school_credit'] : $orderInfo['order_amount'],
            'give_integral' => 0,
        ...
        foreach ($orderGoods as $key => $goods) {
            if ($goods['is_gift'] == 0) {
                ...
                $orderData['goods'][$idKey] = [
                    ...
                    'shop_price' => $goods['goods_price'],
                    'exchange_integral' => $goods['use_integral'],
                    'exchange_price' => $orderInfo['order_type'] == 5 ? $goods['school_credit'] : $goods['member_goods_price'],
                    'original_img' => $goods['original_img'],
                    'original_img_new' => getFullPath($goods['original_img']),
...
//                }
            ...
            foreach ($payList as $k => $list) {
                ...
                $goodsList[$k] = [
                    ...
                    'goods_sn' => $goods['goods_sn'],
                    'goods_name' => $goods['goods_name'],
                    'goods_remark' => $goods['goods_remark'] ?? $goods['spec_key_name'] ?? $list['spec_key_name'] ?? '',
                    'spec_key_name' => $goods['spec_key_name'] ?? $list['spec_key_name'] ??  '',
                    'original_img' => $goods['original_img'],
                    'original_img_new' => getFullPath($goods['original_img']),
    ...
    public function createOrder()
    {
        ...
        try {
            ...
            $payment_where = [
                ...
                'status' => 1,
                'scene' => 3,   // APP支付
                'code' => ['NEQ', 'weixinApp_2']    // 微信支付（韩国购）不显示
            ];
