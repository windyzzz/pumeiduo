...
class Order extends Model
{
    ...
    public function getOrderStatusDetailAttr($value, $data)
    {
        ...
        // 货到付款
        if ('cod' == $data['pay_code']) {
            if (in_array($data['order_status'], [0, 1]) && 0 == $data['shipping_status']) {
                return $data_status_arr['WAITSEND']; //'待发货',
            }
        } else {
            // 非货到付款
            if (0 == $data['pay_status'] && 0 == $data['order_status']) {
                return $data_status_arr['WAITPAY']; //'待支付',
            }
            if (1 == $data['pay_status'] && in_array($data['order_status'], [0, 1]) && 1 != $data['shipping_status']) {
                if (6 == $data['prom_type']) {
        ...
        if ((1 == $data['shipping_status']) && (1 == $data['order_status'])) {
            return $data_status_arr['WAITRECEIVE']; //'待收货',
        }
        if (2 == $data['order_status'] && 1 == $data['is_comment']) {
...
//        	$btn['delivery_confirm'] = '确认收货';
    ...
    public function getOrderButtonAttr($value, $data)
    {
        ...
        // 货到付款
        if ('cod' == $data['pay_code']) {
            if ((0 == $data['order_status'] || 1 == $data['order_status']) && 0 == $data['shipping_status']) {
                $btn_arr['cancel_btn'] = 1; // 取消按钮 (联系客服)
            }
            if (1 == $data['shipping_status'] && 1 == $data['order_status']) { //待收货
                $btn_arr['receive_btn'] = 1;  // 确认收货
                $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
            }
        } // 非货到付款
        else {
            if (0 == $data['pay_status'] && 0 == $data['order_status']) { // 待支付
                $btn_arr['pay_btn'] = 1; // 去支付按钮
                $btn_arr['cancel_btn'] = 1; // 取消按钮
            }
            if (1 == $data['pay_status'] && $data['order_status'] < 2 && 0 == $data['shipping_status']) {
                if (6 == $data['prom_type']) {
            ...
            if (1 == $data['pay_status'] && 1 == $data['order_status'] && 1 == $data['shipping_status']) {
                $btn_arr['receive_btn'] = 1;  // 确认收货
            }
        }
        ...
        if (2 == $data['order_status']) {
            if (0 == $data['is_comment']) {
                $btn_arr['comment_btn'] = 1;
            }  // 评价按钮
            $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
        }
        if ($data['shipping_status'] > 0 && 1 == $data['order_status']) {
            $btn_arr['shipping_btn'] = 1; // 查看物流
        }
        if (2 == $data['shipping_status'] && 1 == $data['order_status']) {
            $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
        }
        if (3 == $data['order_status'] && (1 == $data['pay_status'] || 4 == $data['pay_status'])) {
    ...
    public function getVirtualOrderButtonAttr($value, $data)
    {
        ...
        if (!empty($vr_order_code)) {
            if (1 == $data['pay_status'] && $data['order_status'] < 2 && 1 != $vr_order_code['vr_state'] && $vr_order_code['refund_lock'] < 1) {
                if ($vr_order_code['vr_indate'] > time()) {
                    $Virtual_btn_arr['cancel_btn'] = 2; // 已支付取消按钮
                }
                if ($vr_order_code['vr_indate'] < time() && 1 == $vr_order_code['vr_invalid_refund']) {
                    $Virtual_btn_arr['cancel_btn'] = 2; // 已支付取消按钮
    ...
    {
        $regions = Db::name('region2')->where('id', 'IN', [$data['province'], $data['city'], $data['district'], $data['twon']])->order('level desc')->select();
        $address = '';
        if ($regions) {
