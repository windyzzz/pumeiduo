...
class WxCommon
{
    ...
    protected function httpRequest($url, $method = 'GET', $fields = [])
    {
        ...
        if ('GET' == $method && !empty($fields)) {
            ...
            $url = $url.(false === strpos($url, '?') ? '?' : '&').$fields;
        }
    ...
    {
        if (is_string($value) && 0 === strpos($value, '@') && is_file(realpath(ltrim($value, '@')))) {
            return true;
        }
    ...
    protected function requestAndCheck($url, $method = 'GET', $fields = [], $result_json = true)
    {
        ...
        if ('GET' === strtoupper($method) && empty($wxdata)) {
            if ($this->debug) {
