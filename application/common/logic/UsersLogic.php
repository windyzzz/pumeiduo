...
//            $user['level_name'] = $levelName;
    ...
    public function thirdLogin($data = [], $source = 1)
    {
        ...
        if (!$user) {
            ...
            $invite = TokenLogic::getValue('invite', $data['token']);
            if ($invite > 0) {
                $userInvite = M('users')->where(['user_id' => $user['user_id']])->field('first_leader, invite_uid')->find();
...
//            unset($condition[$field]);
    ...
    public function add_address($user_id, $address_id = 0, $data)
    {
        ...
        $post['zipcode'] = M('region2')->where(['id' => $this->userAddress['district']])->value('zipcode');
        if (!$post['address']) {
