...
class Team
{
    ...
    public function buy()
    {
        if (empty($this->teamActivity) || 1 != $this->teamActivity['status']) {
            throw new TpshopException('拼团购买商品', 0, ['status' => 0, 'msg' => '该商品拼团活动不存在或者已下架', 'result' => '']);
        }
        if (1 == $this->teamActivity['is_lottery']) {
        ...
        if (empty($goods) || 1 != $goods['is_on_sale'] || 6 != $goods['prom_type']) {
            throw new TpshopException('拼团购买商品', 0, ['status' => 0, 'msg' => '该商品拼团活动不存在或者已下架', 'result' => '']);
        }
        if ($this->teamActivity['item_id'] > 0) {
            ...
            if (empty($spec_goods_price) || 6 != $spec_goods_price['prom_type']) {
                throw new TpshopException('拼团购买商品', 0, ['status' => 0, 'msg' => '该商品拼团活动不存在或者已下架', 'result' => '']);
            }
        ...
        if (0 != $this->teamActivity['buy_limit'] && $this->buyNum > $this->teamActivity['buy_limit']) {
            throw new TpshopException('拼团购买商品', 0, ['status' => 0, 'msg' => '购买数已超过该活动单次购买限制数('.$this->teamActivity['buy_limit'].')', 'result' => '']);
        }
        if ($this->foundId) {
            if (empty($this->teamFound) || 1 != $this->teamFound['status']) {
            ...
            if (2 == $this->teamActivity['team_type']) {
                //抽奖团，只能拼一次团
