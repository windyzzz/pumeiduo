...
class WechatLogic
{
    private static $wx_user = null;
    private static $wechat_obj;
    ...
    private function handleSubscribeEvent($msg)
    {
        ...
        if ('event' != $msg['MsgType'] || 'subscribe' != $msg['Event']) {
            exit('不是关注事件');
        }
        ...
        if (!Db::name('oauth_users')->where('openid', $openid)->find()) {
            ...
            if (!empty($msg['EventKey'])) {
                $userData['invite_uid'] = $userData['first_leader'] = substr($msg['EventKey'], strlen('qrscene_'));
    ...
    private function createReplyMsg($from, $to, $type, $data = [])
    {
        ...
        $resultStr = '';
        if (WxReply::MSG_TEXT == $reply->msg_type && $reply['data']) {
            $resultStr = self::$wechat_obj->createReplyMsgOfText($from, $to, $reply['data']);
        } elseif (WxReply::MSG_NEWS == $reply->msg_type) {
    ...
    {
        if (!$keyword) {
            $resultStr = self::$wechat_obj->createReplyMsgOfCustomer($from, $to);
            exit($resultStr);
    ...
    public function deleteSingleNews($news_id)
    {
        ...
        if (1 == count($news->wx_material->wx_news)) {
    ...
    {
        if (false === strpos($url, 'weixin_mp_image/')) {
            return;
        }
    ...
    {
        if (WxReply::TYPE_KEYWORD == $data['type']) {
            $rules = [
                ['type', 'require', '回复类型必需'],
    ...
    public function updateTextAutoReply($reply_id, $data)
    {
        ...
        $with = (WxReply::TYPE_KEYWORD == $data['type']) ? 'wxKeywords' : [];
    ...
    public function deleteAutoReply($reply_id)
    {
        ...
        if (WxReply::TYPE_KEYWORD == $reply->type) {
