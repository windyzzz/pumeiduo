...
class EditorLogic
{
    ...
    public function waterImage($img_path)
    {
        ...
        if (1 == $water['is_mark'] && $image->width() > $water['mark_width'] && $image->height() > $water['mark_height']) {
            if ('text' == $water['mark_type']) {
                $ttf = './hgzb.ttf';
                if (file_exists($ttf)) {
    ...
    private function uploadWechatImage($save_path, $return_url)
    {
        ...
        } elseif ('weixin_mp_news/' == $save_path) {
            $wechat = new \app\common\logic\wechat\WechatUtil();
            $news_img_url = $wechat->uploadNewsImage('.'.$return_url);
            if (false === $news_img_url) {
                $state = $wechat->getError();
            } else {
                WxMaterial::create([
