...
class File
{
    ...
    {
        $size_text = [' B', ' KB', ' MB', ' GB', ' TB', ' PB', ' EB', ' ZB', ' YB'];
     ...
     * @return array 所有文件路径
     */
    public static function getFiles($path, &$files = [], $preg = "/\.(gif|jpeg|jpg|png|bmp)$/i")
    {
        if (!is_dir($path)) {
            return null;
        }
    ...
    {
        $zip = new \ZipArchive();
        if (true !== $zip->open(DB_Backup_PATH.'Zip/'.$file)) {
            return false;
        }
