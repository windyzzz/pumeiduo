...
class Admin extends Base
{
    ...
    public function modify_pwd()
    {
        ...
        if (IS_POST) {
            ...
            } elseif ($newPwd != $new2Pwd) {
                exit(json_encode(['status' => -1, 'msg' => '两次密码不一致']));
            }
            $row = M('admin')->where('admin_id', $admin_id)->save(['password' => $enNewPwd]);
            if ($row) {
    ...
    public function adminHandle()
    {
        ...
        if ('del' == $data['act'] && $data['admin_id'] > 1) {
    ...
    public function roleSave()
    {
        ...
        if ($r) {
            adminLog('管理角色');
            $this->success('操作成功!', U('Admin/Admin/role_info', ['role_id' => $data['role_id']]));
        } else {
    ...
    public function supplierHandle()
    {
        ...
        if ('add' == $data['act']) {
            ...
            if ($count) {
                ...
                if (!empty($data['admin_id'])) {
                    ...
                    M('admin')->where(['suppliers_id' => $admin_data['suppliers_id']])->save(['suppliers_id' => 0]);
                    M('admin')->where(['admin_id' => $data['admin_id']])->save($admin_data);
                }
            }
        }
        //改
        if ('edit' == $data['act'] && $data['suppliers_id'] > 0) {
        ...
        //删
        if ('del' == $data['act'] && $data['suppliers_id'] > 0) {
            $r = $suppliers_model->where('suppliers_id', $data['suppliers_id'])->delete();
            M('admin')->where(['suppliers_id' => $data['suppliers_id']])->save(['suppliers_id' => 0]);
            if ($r) {
                respose(1);
            } else {
                respose('删除失败');
