...
//                $this->error('检测到有一个备份任务正在执行，请稍后再试！');
                return json(['info' => '检测到有一个备份任务正在执行，请稍后再试！', 'status' => 0, 'url' => '']);
            }
            //创建锁文件
...
//            is_writeable($config['path']) || $this->error('备份目录不存在或不可写，请检查后重试！');
            if (!is_writeable($config['path'])) {
                return json(['info' => '备份目录不存在或不可写，请检查后重试！', 'status' => 0, 'url' => '']);
            }
...
//                $this->success('初始化成功！', '', array('tables' => $tables, 'tab' => $tab));
                return json(['tables' => $tables, 'tab' => $tab, 'info' => '初始化成功！', 'status' => 1, 'url' => '']);
            }
//                $this->error('初始化失败，备份文件创建失败！');
            return json(['info' => '初始化失败，备份文件创建失败！', 'status' => 0, 'url' => '']);
            ...
            if (false === $start) { //出错
//                $this->error('备份出错！');
                return json(['info' => '备份出错！', 'status' => 0, 'url' => '']);
            } elseif (0 === $start) { //下一表
                if (isset($tables[++$id])) {
                    $tab = ['id' => $id, 'start' => 0];
//                    $this->success('备份完成！', '', array('tab' => $tab));
                    return json(['tab' => $tab, 'info' => '备份完成！', 'status' => 1, 'url' => '']);
                }   //备份完成，清空缓存
...
//                    $this->success('备份完成！');
                return json(['info' => '备份完成！', 'status' => 1, 'url' => '']);
            }
            $tab = ['id' => $id, 'start' => $start[0]];
            $rate = floor(100 * ($start[0] / $start[1]));
//                $this->success("正在备份...({$rate}%)", '', array('tab' => $tab));
            return json(['tab' => $tab, 'info' => "正在备份...({$rate}%)", 'status' => 1, 'url' => '']);
        }  //出错
//            $this->error('参数错误！');
        return json(['info' => '参数错误！', 'status' => 0, 'url' => '']);
    }
    ...
    public function restore()
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\d{8,8}-\d{6,6}-\d+\.sql(?:\.gz)?$/', $name)) {
                ...
                $info['compress'] = ('sql' === $info['extension']) ? '-' : $info['extension'];
...
//                    array_map("unlink", glob($path));
    ...
    public function region()
    {
        ...
        if (0 == $parent_id) {
            $parent = ['id' => 0, 'name' => '中国省份地区', 'level' => 0];
        } else {
        ...
        $region = M('region2')->where('parent_id', $parent_id)->select();
     ...
     * @return array
     */
    public function getParentRegionList($parent_id)
    {
        $names = [];
        $region = M('region2')->where(['id' => $parent_id])->find();
    ...
    public function regionHandle()
    {
        ...
        if (empty($id)) {
            ...
            if (empty($data['name'])) {
                ...
                $res = M('region2')->where('parent_id = '.$data['parent_id']." and name='".$data['name']."'")->find();
                if (empty($res)) {
                    M('region2')->cache(true)->add($data);
            ...
            M('region2')->where("id=$id or parent_id=$id")->cache(true)->delete();
            $this->success('操作成功', $referurl);
        }
    }
