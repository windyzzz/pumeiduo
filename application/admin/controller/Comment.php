...
class Comment extends Base
{
    ...
    public function detail()
    {
        ...
        $reply = M('comment')->where(['parent_id' => $id])->select(); // 评论回复列表
    ...
    public function commentHandle()
    {
        $type = I('post.type');
        $ids = I('post.ids', '');
        if (!in_array($type, ['del', 'show', 'hide']) || empty($ids)) {
    ...
    public function ajax_ask_list()
    {
        ...
        $consult_type = [0 => '默认咨询', 1 => '商品咨询', 2 => '支付咨询', 3 => '配送', 4 => '售后'];
    ...
    {
        $id = I('id/d', 0);
        $res = M('goods_consult')->where(['id' => $id])->find();
        if (!$res) {
        ...
        $reply = M('goods_consult')->where(['parent_id' => $id])->select(); // 咨询回复列表
        $this->assign('id', $id);
        $this->assign('comment', $res);
        $this->assign('reply', $reply);
    ...
    public function ask_handle()
    {
        ...
        if ('show' == $type) {
            $row = M('goods_consult')->where('id', 'IN', $selected_id)->save(['is_show' => 1]);
        }
        if ('hide' == $type) {
            $row = M('goods_consult')->where('id', 'IN', $selected_id)->save(['is_show' => 0]);
        }
        if (false !== $row) {
