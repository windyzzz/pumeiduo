...
class System extends Base
{
    ...
    public function ClearGoodsHtml()
    {
        ...
        if (unlink("./Application/Runtime/Html/Home_Goods_goodsInfo_{$goods_id}.html")) {
            ...
            $json_arr = ['status' => -1, 'msg' => '未能清除缓存', 'result' => ''];
        }
        $json_str = json_encode($json_arr);
        exit($json_str);
    }
...
//         }
    ...
    public function ajax_get_action()
    {
        ...
        foreach ($methods as $method) {
            if ($method->class == $className) {
                if ('__construct' != $method->name && '_initialize' != $method->name) {
    ...
    public function edit_right()
    {
        ...
        if (IS_POST) {
            ...
            if (!empty($data['id'])) {
                M('system_menu')->where(['id' => $data['id']])->save($data);
            } else {
                if (M('system_menu')->where(['type' => $data['type'], 'name' => $data['name']])->count() > 0) {
                    $this->error('该权限名称已添加，请检查', U('System/right_list'));
                }
                unset($data['id']);
                M('system_menu')->add($data);
            }
            $this->success('操作成功', U('System/right_list', ['type' => $data['type']]));
            exit;
        }
        $id = I('id');
        if ($id) {
            $info = M('system_menu')->where(['id' => $id])->find();
